...
import { MusicologyMetadataForm, DEFAULT_HISTORICAL } from '@/components/musicology-metadata-form';
...
export default function MusicologyPage() {
  ...
  const [filterBy, setFilterBy] = useState<'all' | 'historical' | 'ethnomusicology' | 'systematic' | 'popular'>('all');
  ...
  const [researchProjects, setResearchProjects] = useState<ResearchProject[]>([
    {
      ...
      description: 'Analysis of historical performance practices in Baroque keyboard music',
    ...
    },
    {
      ...
      description: 'Ethnomusicological study of traditional drumming patterns in West Africa',
    ...
    },
    {
      ...
      description: 'Systematic analysis of frequency content in electronic music production',
  ...
  const [analyses] = useState<Analysis[]>([
    {
      ...
      results: {
        keyFindings: ['Identified 3 distinct rhythmic patterns', 'Found correlation with social context', 'Discovered regional variations'],
        confidence: 0.87,
        dataPoints: 1250
      }
    },
  ...
  const handleUpdateProject = useCallback((projectId: string, updates: Partial<ResearchProject>) => {
    ...
    if (selectedProject?.id === projectId) {
      setSelectedProject(prev => prev ? { ...prev, ...updates, updatedAt: new Date() } : null);
    }
  ...
  const handleStartAnalysis = useCallback((type: Analysis['type']) => {
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <span>{project.researchers.length} researchers</span>
        ...
        <TabsContent value="analyses" className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => handleStartAnalysis('transcription')}>
                  <MusicIcon className="h-4 w-4 mr-2" />
                  Transcription Analysis
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleStartAnalysis('cultural')}>
                  <Globe className="h-4 w-4 mr-2" />
                  Cultural Analysis
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleStartAnalysis('comparative')}>
                  <GitBranch className="h-4 w-4 mr-2" />
                  Comparative Analysis
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleStartAnalysis('statistical')}>
              ...
              <Card key={analysis.id}>
                <CardHeader>
                  ...
                  <CardDescription>
                    ...
                    {analysis.completedAt && ` • Completed ${analysis.completedAt.toLocaleDateString()}`}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                      ...
                      {analysis.status === 'completed' && analysis.results && (
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center justify-between">
                            ...
                            <span>{(analysis.results.confidence * 100).toFixed(1)}%</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span>Data Points</span>
                            <span>{analysis.results.dataPoints.toLocaleString()}</span>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {analysis.results?.keyFindings && (
                      <div>
                        ...
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {analysis.results.keyFindings.map((finding, index) => (
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold">{selectedProject.files}</div>
                      <div className="text-sm text-muted-foreground">Files</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold">{selectedProject.analyses}</div>
                      <div className="text-sm text-muted-foreground">Analyses</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold">{selectedProject.researchers.length}</div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Upload audio files and documents to get started</p>
                  </div>
                </CardContent>
              </Card>
            </div>
