...
export default function StemSeparationPage() {
  ...
  const [selectedResult, setSelectedResult] = useState<SeparationResult | null>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentPlayingStem, setCurrentPlayingStem] = useState<string | null>(null);
  ...
  const exportStem = useCallback((stem: StemTrack, fileName: string) => {
    // Simulate stem export
    const blob = new Blob(['mock-audio-data'], { type: 'audio/wav' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName.replace(/\.[^/.]+$/, '')}_${stem.name.toLowerCase()}.wav`;
    a.click();
  ...
  const exportAllStems = useCallback((result: SeparationResult) => {
    ...
    toast({
      ...
      description: `Exported ${result.stems.filter(s => s.confidence > 0.5).length} stems`,
    });
  }, [toast, exportStem]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Isolate individual instruments from audio files using advanced frequency analysis
          </p>
        </div>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSeparationSettings(prev => ({
                    ...prev,
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={key} className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={separationSettings[key as keyof typeof separationSettings]}
                      onCheckedChange={(checked) => setSeparationSettings(prev => ({
                        ...prev,
                        [key]: checked
                      }))}
                    />
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            {Math.floor(result.originalDuration / 60)}:{(result.originalDuration % 60).toFixed(0).padStart(2, '0')}
                        ...
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge 
                                variant={stem.confidence > 0.8 ? "default" : stem.confidence > 0.6 ? "secondary" : "outline"}
                                className="text-xs"
                              >
                            ...
                            <div className="text-xs text-muted-foreground">
                              {stem.frequencyRange[0]}-{stem.frequencyRange[1]} Hz
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            <Button
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => exportStem(stem, selectedResult.fileName)}
                            >
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-4 text-sm">
                  <div>
                    ...
                    <p>{Math.floor(selectedResult.originalDuration / 60)}:{(selectedResult.originalDuration % 60).toFixed(0).padStart(2, '0')}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p>{(selectedResult.overallConfidence * 100).toFixed(1)}%</p>
                  </div>
                  <div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium mb-2">Separating audio stems...</p>
