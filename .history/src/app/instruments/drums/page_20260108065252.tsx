...
export default function DrumsPage() {
  ...
  const [selectedResult, setSelectedResult] = useState<DrumAnalysisResult | null>(null);
  ...
  const startDrumAnalysis = useCallback(async () => {
    ...
    try {
      ...
      for (const file of uploadedFiles) {
        ...
        const generateDrumHits = (instrument: string, count: number): DrumHit[] => {
        ...
        const mockResult: DrumAnalysisResult = {
          ...
          timeSignature: ['4/4', '3/4', '6/8', '5/4'][Math.floor(Math.random() * 4)],
          ...
          patterns: [
            {
              ...
              tempo: 120,
              duration: 8,
              hits: generateDrumHits('kick', 16).concat(generateDrumHits('snare', 8)),
              style: 'Rock',
              complexity: 3
            },
            {
              ...
              tempo: 95,
              duration: 4,
              hits: generateDrumHits('hiHat', 32).concat(generateDrumHits('snare', 8)),
              style: 'Funk',
              complexity: 6
            }
          ],
          ...
          technique: {
            handTechnique: ['Matched grip', 'Traditional grip', 'Moles technique'].slice(0, Math.floor(Math.random() * 2) + 1),
            footTechnique: ['Heel-down', 'Heel-up', 'Slide technique'].slice(0, Math.floor(Math.random() * 2) + 1),
            dynamics: ['Ghost notes', 'Accents', 'Crescendo', 'Diminuendo'].slice(0, Math.floor(Math.random() * 3) + 1),
            timing: Math.random() * 0.1 + 0.9,
            consistency: Math.random() * 0.2 + 0.8
          },
          
          style: {
            primaryGenre: ['Rock', 'Jazz', 'Funk', 'Blues', 'Metal', 'Latin'][Math.floor(Math.random() * 6)],
            influences: ['Rock', 'Jazz', 'Funk', 'Blues', 'Latin', 'Afro-Cuban'].slice(0, Math.floor(Math.random() * 3) + 1),
            era: ['1950s', '1960s', '1970s', '1980s', '1990s', '2000s', 'Modern'][Math.floor(Math.random() * 7)],
  ...
  const exportDrumTab = useCallback((result: DrumAnalysisResult) => {
    const drumTabData = {
      title: result.fileName,
      tempo: result.tempo,
      timeSignature: result.timeSignature,
      duration: result.duration,
      components: Object.entries(result.components).map(([instrument, hits]) => ({
        instrument,
        hits: hits.map(hit => ({
          time: hit.startTime,
    ...
    const blob = new Blob([JSON.stringify(drumTabData, null, 2)], { type: 'application/json' });
  ...
  const exportMIDI = useCallback((result: DrumAnalysisResult) => {
    const midiData = JSON.stringify({
      tempo: result.tempo,
      timeSignature: result.timeSignature,
      hits: Object.values(result.components).flat().map(hit => ({
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Specialized analysis for drum recordings including pattern detection, technique analysis, and rhythm transcription
          </p>
        </div>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {[
                    { key: 'enablePatternDetection', label: 'Pattern Detection' },
                    { key: 'enableFillDetection', label: 'Fill Detection' },
                    { key: 'enableTechniqueAnalysis', label: 'Technique Analysis' },
                    { key: 'enableStyleClassification', label: 'Style Classification' },
                    { key: 'enableTempoTracking', label: 'Tempo Tracking' }
                  ].map(({ key, label }) => (
                    <div key={key} className="flex items-center justify-between">
                      <span className="text-sm">{label}</span>
                      <Switch
                        checked={drumSettings[key as keyof DrumSettings]}
                        onCheckedChange={(checked) => setDrumSettings(prev => ({
                          ...prev,
                          [key]: checked
                        }))}
                      />
              ...
              <div>
                ...
                <Slider
                  ...
                  onValueChange={([value]) => setDrumSettings(prev => ({
                    ...prev,
                    sensitivity: value
                  }))}
                  max={1}
              ...
              <div>
                <label className="text-sm font-medium">Minimum Hit Duration</label>
                <Slider
            ...
            <CardContent>
              {results.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Upload drum recordings and start analysis</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {results.map((result) => (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            {Object.values(result.components).flat().length} hits
                          </div>
                        </div>
                ...
                <Tabs value={currentTab} onValueChange={setCurrentTab}>
                  ...
                  <TabsContent value="overview" className="space-y-4">
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-bold">{Object.values(selectedResult.components).flat().length}</p>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-bold">{selectedResult.timeSignature}</p>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span>{getComplexityLabel(selectedResult.style.complexity)}</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Performance Quality</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                          <div className="flex justify-between text-sm">
                            <span>Timing:</span>
                            <span>{getTimingLabel(selectedResult.technique.timing)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Consistency:</span>
                            <span>{(selectedResult.technique.consistency * 100).toFixed(1)}%</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Patterns:</span>
                            <span>{selectedResult.patterns.length} detected</span>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <TabsContent value="components" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      {Object.entries(selectedResult.components).map(([instrument, hits]) => (
                        <Card key={instrument}>
                          <CardHeader>
                            <CardTitle className="text-sm capitalize">{instrument}</CardTitle>
                          </CardHeader>
                          <CardContent>
                            <div className="space-y-2">
                              <div className="flex justify-between text-sm">
                              ...
                              <div className="flex justify-between text-sm">
                                ...
                                <span>
                                  {hits.length > 0 
                                    ? (hits.reduce((sum, hit) => sum + hit.velocity, 0) / hits.length).toFixed(1)
                                    : 'N/A'}
                                </span>
                              </div>
                              <div className="flex justify-between text-sm">
                              ...
                              <div className="mt-2">
                                <Progress 
                                  value={(hits.length / Math.max(...Object.values(selectedResult.components).map(h => h.length))) * 100} 
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          {Object.entries(selectedResult.components).map(([instrument, hits]) => {
                            const totalHits = Object.values(selectedResult.components).flat().length;
                            const percentage = (hits.length / totalHits) * 100;
                            return (
                              <div key={instrument} className="flex items-center gap-2">
                                <span className="text-sm w-16 capitalize">{instrument}:</span>
                            ...
                            {selectedResult.patterns.map((pattern, idx) => (
                              <div key={idx} className="p-2 border rounded">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {pattern.timeSignature} • {pattern.tempo} BPM • {pattern.duration}s
                                </div>
                                <div className="text-xs text-muted-foreground">
                          ...
                          <div className="space-y-2">
                            {selectedResult.fills.map((fill, idx) => (
                              <div key={idx} className="p-2 border rounded">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {fill.startTime.toFixed(1)}s - {fill.endTime.toFixed(1)}s
                                </div>
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {fill.instruments.map((instrument, instrumentIdx) => (
                                    <Badge key={instrumentIdx} variant="secondary" className="text-xs">
                                      {instrument}
                                    </Badge>
                                  ))}
                                </div>
                  ...
                  <TabsContent value="technique" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Hand Technique</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex flex-wrap gap-1">
                            {selectedResult.technique.handTechnique.map((technique, idx) => (
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Foot Technique</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex flex-wrap gap-1">
                            {selectedResult.technique.footTechnique.map((technique, idx) => (
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-sm">Dynamics & Articulation</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="flex flex-wrap gap-1">
                          {selectedResult.technique.dynamics.map((dynamic, idx) => (
                            <Badge key={idx} variant="secondary" className="text-xs">
                              {dynamic}
                            </Badge>
                          ))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Style Influences</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex flex-wrap gap-1">
                            {selectedResult.style.influences.map((influence, idx) => (
                              <Badge key={idx} variant="outline" className="text-xs">
                                {influence}
                              </Badge>
                            ))}
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span>{(selectedResult.technique.timing * 100).toFixed(1)}%</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Consistency:</span>
                            <span>{(selectedResult.technique.consistency * 100).toFixed(1)}%</span>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium mb-2">Analyzing drum recordings...</p>
