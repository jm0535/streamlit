...
export default function GuitarPage() {
  ...
  const [selectedResult, setSelectedResult] = useState<GuitarAnalysisResult | null>(null);
    ...
    try {
      ...
      for (const file of uploadedFiles) {
        ...
        const generateGuitarNotes = (): GuitarNote[] => {
          ...
          const standardTuning = [82.41, 110.00, 146.83, 196.00, 246.94, 329.63]; // E-A-D-G-B-E
        ...
        const mockResult: GuitarAnalysisResult = {
          ...
          tuning: ['E', 'A', 'D', 'G', 'B', 'E'],
          keySignature: ['E Major', 'A Major', 'D Major', 'G Major', 'C Major'][Math.floor(Math.random() * 5)],
          tempo: Math.floor(Math.random() * 60) + 80,
          timeSignature: ['4/4', '3/4', '6/8'][Math.floor(Math.random() * 3)],
          technique: {
            strummingPatterns: ['Down-strum', 'Up-strum', 'Fingerstyle', 'Hybrid picking'].slice(0, Math.floor(Math.random() * 3) + 1),
            pickingStyle: ['Alternate picking', 'Economy picking', 'Sweep picking', 'Tremolo'].slice(0, Math.floor(Math.random() * 2) + 1),
            chords: [
              { startTime: 0, name: 'E Major', fingering: [0, 2, 2, 1, 0, 0], type: 'Major' },
              { startTime: 4, name: 'A Major', fingering: [0, 0, 2, 2, 2, 0], type: 'Major' },
              { startTime: 8, name: 'D Major', fingering: [0, 0, 0, 2, 3, 2], type: 'Major' }
            ],
            scales: [
          ...
          playingStyle: {
            genre: ['Rock', 'Blues', 'Jazz', 'Folk', 'Classical', 'Metal'][Math.floor(Math.random() * 6)],
            complexity: Math.random() * 5 + 3,
            speed: Math.random() * 100 + 60,
            articulation: ['Staccato', 'Legato', 'Vibrato', 'Bend', 'Slide'].slice(0, Math.floor(Math.random() * 3) + 1)
          },
          equipment: {
            estimatedGuitarType: ['Electric', 'Acoustic', 'Classical', '12-String'][Math.floor(Math.random() * 4)],
            effects: ['Distortion', 'Reverb', 'Delay', 'Chorus', 'Clean'].slice(0, Math.floor(Math.random() * 3) + 1),
            amplification: ['Tube Amp', 'Solid State', 'Modeling Amp', 'Acoustic Pickup'][Math.floor(Math.random() * 4)]
          }
        };
  ...
  const exportGuitarTab = useCallback((result: GuitarAnalysisResult) => {
    const tabData = {
    ...
    a.href = url;
    a.download = `${result.fileName.replace(/\.[^/.]+$/, '')}_guitar_tab.json`;
    a.click();
  ...
  const exportMIDI = useCallback((result: GuitarAnalysisResult) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Specialized analysis for guitar recordings including chord detection, tablature generation, and technique analysis
          </p>
        </div>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { key: 'enableGenreClassification', label: 'Genre Classification' }
                  ].map(({ key, label }) => (
                    <div key={key} className="flex items-center justify-between">
                      <span className="text-sm">{label}</span>
                      <Switch
                        checked={guitarSettings[key as keyof GuitarSettings]}
                        onCheckedChange={(checked) => setGuitarSettings(prev => ({
                          ...prev,
                          [key]: checked
                        }))}
                      />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setGuitarSettings(prev => ({
                    ...prev,
                    stringCount: parseInt(e.target.value)
                  }))}
              ...
              <div>
                ...
                <Slider
                  ...
                  onValueChange={([value]) => setGuitarSettings(prev => ({
                    ...prev,
                    sensitivity: value
                  }))}
                  max={1}
              ...
              <div>
                <label className="text-sm font-medium">Minimum Note Duration</label>
                <Slider
            ...
            <CardContent>
              {results.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Upload guitar recordings and start analysis</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {results.map((result) => (
                ...
                <Tabs value={currentTab} onValueChange={setCurrentTab}>
                  ...
                  <TabsContent value="overview" className="space-y-4">
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-lg font-bold">{selectedResult.tuning.join('-')}</p>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Playing Style</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span>{getComplexityLabel(selectedResult.playingStyle.complexity)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Speed:</span>
                            <span>{getSpeedLabel(selectedResult.playingStyle.speed)}</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span>{[...new Set(selectedResult.notes.map(n => n.string))].length}/6 strings</span>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
                  ...
                  <TabsContent value="technique" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Strumming Patterns</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex flex-wrap gap-1">
                            {selectedResult.technique.strummingPatterns.map((pattern, idx) => (
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Picking Style</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="flex flex-wrap gap-1">
                            {selectedResult.technique.pickingStyle.map((style, idx) => (
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="flex flex-wrap gap-1">
                          {selectedResult.playingStyle.articulation.map((articulation, idx) => (
                            <Badge key={idx} variant="secondary" className="text-xs">
                              {articulation}
                            </Badge>
                          ))}
                  ...
                  <TabsContent value="chords" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Detected Chords</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-2">
                            {selectedResult.technique.chords.map((chord, idx) => (
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Detected Scales</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-2">
                            {selectedResult.technique.scales.map((scale, idx) => (
                              <div key={idx} className="p-2 border rounded">
                                <div className="flex items-center justify-between mb-1">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {scale.startTime.toFixed(1)}s - {scale.endTime.toFixed(1)}s
                                </div>
                              </div>
                            ))}
                          </div>
                  ...
                  <TabsContent value="equipment" className="space-y-4">
                    <div className="grid grid-cols-3 gap-4">
                      <Card>
                        ...
                        <CardContent>
                          <div className="text-center">
                            ...
                            <p className="font-medium">{selectedResult.equipment.estimatedGuitarType}</p>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent>
                          <div className="flex flex-wrap gap-1 justify-center">
                            {selectedResult.equipment.effects.map((effect, idx) => (
                              <Badge key={idx} variant="secondary" className="text-xs">
                                {effect}
                              </Badge>
                            ))}
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-sm">Amplification</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="text-center">
                            <Volume2 className="h-8 w-8 mx-auto mb-2 text-primary" />
                            <p className="font-medium">{selectedResult.equipment.amplification}</p>
                          </div>
                        </CardContent>
                      </Card>
                    </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium mb-2">Analyzing guitar recordings...</p>
