...
export function AudioEffectsProcessor() {
  ...
  const updateEffectParameter = useCallback((effectName: string, parameter: string, value: number) => {
    setEffects(prev => prev.map(effect => 
      effect.name === effectName 
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant={state.quality === 'high' ? 'default' : state.quality === 'medium' ? 'secondary' : 'outline'}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">
                {state.isProcessing ? 'Processing' : state.isRecording ? 'Recording' : state.isPlaying ? 'Playing' : 'Idle'}
              </div>
            </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(state.inputLevel)}%</div>
              <div className="text-sm text-muted-foreground">Input Level</div>
            </div>
            
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(state.outputLevel)}%</div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {state.isProcessing && (
              <Button onClick={resetProcessor} variant="destructive" className="flex-1">
            ...
            {state.isPlaying && (
              <Button onClick={stopAudio} variant="destructive" className="flex-1">
                <Square className="h-4 w-4 mr-2" />
                Stop
              </Button>
            )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">
                {state.quality === 'high' ? 'Best results, slower processing' : 
                 state.quality === 'medium' ? 'Balanced speed and quality' : 
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() => setState(prev => ({ ...prev, quality: 'medium' }))}
              >
                Medium
              </Button>
              <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {effects.map((effect) => (
          <Card key={effect.name} className={`transition-all duration-200 ${
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {effect.icon}
                  <CardTitle className="text-lg capitalize">{effect.name}</CardTitle>
                </div>
                <Button
              ...
              {Object.entries(effect.parameters).map(([param, value]) => (
                <div key={param} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span>
                      {param.includes('time') ? `${value}ms` : 
                       param.includes('threshold') ? `${value}dB` : 
                       param.includes('ratio') ? `${value}:1` : 
                       param.includes('frequency') ? `${value}Hz` :
                       `${value}%`}
                    </span>
                  </div>
                  <Slider
                    value={[value]}
                    onValueChange={([newValue]) => updateEffectParameter(effect.name, param, newValue)}
                    max={param.includes('threshold') ? 0 : param.includes('ratio') ? 20 : param.includes('frequency') ? 20000 : 100}
                    min={param.includes('threshold') ? -60 : param.includes('frequency') ? 20 : 0}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Quickly apply common effect combinations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
...
export function AdvancedAudioAnalyzer() {
  ...
  const [analysisType, setAnalysisType] = useState<'spectrum' | 'spectrogram' | 'waveform' | 'phase'>('spectrum');
  ...
  useEffect(() => {
    if (isAnalyzing) {
      const interval = setInterval(() => {
        ...
        // Calculate RMS level
        const rms = Math.sqrt(newData.reduce((sum, val) => sum + val * val, 0) / newData.length);
        setRmsLevel(rms);
      }, 50);
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(['spectrum', 'spectrogram', 'waveform', 'phase'] as const).map(type => (
                <Button
                  key={type}
        ...
        <div className="space-y-4">
          <div className="flex items-end justify-between h-64 gap-1">
            {frequencyData.map((freq, index) => (
              <div key={index} className="flex-1 flex flex-col items-center">
                ...
                <div 
                  className={`w-full transition-all duration-100 ${getFrequencyColor(freq)}`}
                  style={{ height: `${freq}%` }}
                />
                <div className="text-xs text-muted-foreground mt-1">
                  {index < 10 ? `${index * 100}Hz` : 
                   index < 20 ? `${(index - 10) * 500}Hz` : 
                   index < 30 ? `${(index - 20) * 1000}Hz` : 
                   `${(index - 30) * 2000}Hz`}
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(peakFrequency)} Hz</div>
              <div className="text-sm text-muted-foreground">Peak Frequency</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(frequencyData.reduce((a, b) => a + b, 0) / frequencyData.length)}%</div>
              <div className="text-sm text-muted-foreground">Average Level</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(Math.max(...frequencyData))}%</div>
