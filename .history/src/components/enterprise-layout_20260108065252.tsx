    ...
    items: [
      {
        ...
        href: '/musicology',
        icon: FileText,
        description: 'Research metadata and analysis'
      },
      {
        title: 'Statistics',
        href: '/statistics',
      ...
      return (
        <Link
          ...
          className={cn(
            ...
            isActive ? 'bg-accent text-accent-foreground font-medium nav-link active' : 'nav-link'
          )}
        >
          <item.icon className="h-4 w-4 audio-icon" />
          <span className="truncate">{item.title}</span>
          {item.badge && (
            <Badge variant="secondary" className="ml-auto text-xs badge-secondary">
              {item.badge}
            </Badge>
          )}
        </Link>
